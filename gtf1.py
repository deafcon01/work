...
def dict_to_tf_example(data,
  ...
  """
  img_path = os.path.join(image_subdirectory, re.sub(r"[^\S\n\t]+",'_',data['meta']['imageid']))
  ...
  if 'annotations' in data:#object->annotations
    for obj in data['annotations']:
      ...
      if faces_only:
        xmin = float(data['annotations']['left'])
        xmax = float(data['annotations']['right'])
        ymin = float(data['annotations']['top'])
        ymax = float(data['annotations']['bottom'])
      else:
      ...
      class_name = get_class_name_from_filename(data['annotations']['caption'])
...
def create_tf_record(output_filename,
  ...
  with contextlib2.ExitStack() as tf_record_close_stack:
    ...
    for idx, example in enumerate(examples):
      ...
      data = dataset_util.recursive_parse_xml_to_dict(xml)['all'] #annotation->all
