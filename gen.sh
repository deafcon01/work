...
if [[ ! -d $unzip ]]; then
    ...
    unzip '18th april.zip';
    unzip '22nd april.zip';
...
#awk -F '[\,]' '{ if(($4=="class")||($4=="missing_tooth")||($4=="boneloss")){print $0}}' < data/test_labels1.csv > data/bonelosstest.csv
#cp data/bonelosstrain.csv .
#cp data/bonelosstest.csv .
#awk -F '[\,]' '{if($4=="defect") print $0}' < bonelosstest.csv > testbone.csv
#awk -F '[\,]' '{if($4=="defect") print $0}' < bonelosstrain.csv > trainbone.csv
#awk -F '[\,]' '{if($4=="other") print $0}' < bonelosstest.csv > testmiss.csv
#awk -F '[\,]' '{if($4=="other") print $0}' < bonelosstrain.csv > trainmiss.csv
...
python3 gtf1.py --label_map_path='label_map.pbtxt' --data_dir='data/train' --output_dir='data/TFRecords'
